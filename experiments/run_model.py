...
'''
Description:
    This script is only intended for English and just for testing the data 
    generation efforts.
'''
...
def embedding_model(args, lang, X_train, X_test, y_train, y_test):
    ...
    train_cache_file = Path(
        CACHE_DIR / f'{lang}_{model_id}_train_cache.pickle')
    ...
    evaluate_model(args, SVC(random_state=0, kernel='linear'), lang,
                   train_embeddings, test_embeddings, y_train, y_test, f'{model_id}_svc')


def svc_model(args, lang, X_train, X_test, y_train, y_test):
    ''' Create tf-idf embeddings and evaluate a simple linear model'''
    vectorizer = TfidfVectorizer(stop_words=set())

    # This can be used to get the most informative features from a tfidf 
    # vectorizer.
    # import numpy as np
    ...
    evaluate_model(args, model, lang, X_train, X_test,
                   y_train, y_test, 'tfidf_svc')
...
def main():
    ...
    if args.embeddings:
        embedding_model(args, 'en', X_train, X_test, y_train, y_test)
    if args.svc:
        svc_model(args, 'en', X_train, X_test, y_train, y_test)
    if args.most_frequent:
        most_freq_model(args, 'en', X_train, X_test, y_train, y_test)
