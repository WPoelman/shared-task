...
def create_arg_parser():
    ...
    parser.add_argument('-l', '--target_language', type=str, default='fr',
                        help='The target language(s) to translate into: French (fr) or Italian (it)')
    parser.add_argument('-s', '--source_text', type=str, default='data_experiments/train_base.csv',
...
def main():
    ...
    if args.target_language in ['fr', 'it']:
        ...
        raise ValueError("Please specify one of the following target languages: 'fr' or 'it'.")

    # Get translation system
    if args.translator in ['google', 'microsoft', 'pons']:
        translator = args.translator
    else:
        raise ValueError("Please specify one of the following translators: 'google', 'microsoft', 'pons'.")
