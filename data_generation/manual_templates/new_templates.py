...
class Sentence:
	...
	def output(self):
		...
		sent_label = random.choice(train_templatelist)  # Change to train_templatelist_baseline for original train templates only
				...
				while not (count0 == n_sent and count1 == n_sent):
					...
					if x == relation:
						if x in generated_sent[1]:
							if count1 != n_sent:
								sent_count += 1
								writer.writerow(["templ-{0}".format(sent_count),sent, 1])
							...
							if count0 != n_sent:
								sent_count += 1
								writer.writerow(["templ-{0}".format(sent_count),sent, 0])
