...
def main():
    ...
    same_label_fr = {
        "instruction": "same",
        r"(((J'|Il|Je) (adore|apprécie|utilise|aime|ai rencontré|n' aime pas))|(Je peux comprendre)) (.*) plus que (.*) \.": r"\1 \7 plus que \6 .",
        r"(Il fait confiance|Il ne fait pas confiance) (à|aux) (.*) plus que (à|aux) (.*) \.": r"\1 \4 \5 plus que \2 \3 .",
        r"(((J'|Il|Je) (adore|apprécie|utilise|aime|ai rencontré|n' aime pas))|(Je peux comprendre)) (.*) , je préfère (.*) \.": r"\1 \7 , je préfère \6 .",
        r"(((J'|Il|Je) (adore|apprécie|utilise|aime|ai rencontré|n' aime pas))|(Je peux comprendre)) (.*) , il préfère (.*) \.": r"\1 \7 , il préfère \6 .",
        r"(((J'|Il|Je) (adore|apprécie|utilise|aime|ai rencontré|n' aime pas))|(Je peux comprendre)) (.*) , et aussi (.*) \.": r"\1 \7 , et aussi \6 .",
    }
    ...
    pattern_lists = {
        "en": [same_label_en, one_to_zero_en, zero_to_one_en],
        "fr": [same_label_fr],
        "it": [same_label_it],
    }
    ...
    for lang in args.languages:
        ...
        new_templates_list = [
            [id, sent, label] for id, sent, label in zip(nids, nsentences, nlabels)
        ]
